<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" {{ $attributes }}>
    <circle cx="12" cy="12" r="11" fill="currentColor"/>
    <path fill="none" stroke="white" stroke-width="2" stroke-linecap="round"
          d="M14 8a2.5 2.5 0 0 0-2-1h-1a2 2 0 0 0 0 4h2a2 2 0 0 1 0 4h-1a2.5 2.5 0 0 1-2-1"/>
    <path fill="none" stroke="white" stroke-width="1.5" stroke-linecap="round" d="M12 5.5V7m0 10v1.5"/>
    <path fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
          d="M17 16l3 3-3 3"/>
</svg>
